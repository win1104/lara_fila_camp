                            ...
                            @foreach($posts as $post)
                                <div class="py-8 flex flex-wrap md:flex-nowrap">
                                    <div class="md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col">
                                        <span class="font-semibold title-font text-gray-700">{{ $post->categories->first()?->title ?? '未分類' }}</span>
                                        <span class="text-sm text-gray-500">{{ $post->date }}</span>
                                    </div>
                                    <div class="md:flex-grow">
                                        <p class="text-2xl font-medium text-gray-900 title-font mb-2">{{ $post->title }}</p>
                                        {{-- <p class="leading-relaxed">{!! $post->content !!}</p> --}}
                                        <p class="leading-relaxed">{!! Str::limit($post->content, 300) !!}</p>
                                        <a href="{{ url(app()->getLocale() . '/lists/news/' . $post->slug) }}" class="text-indigo-500 inline-flex items-center mt-4">Learn More
